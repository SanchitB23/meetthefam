...
- **The release-flow gap is closed by construction.** No more workaround — `release/vX.Y.Z` IS the path. Closes the open item at [phase-backlog.md:121](../tasks/phase-backlog.md).
...
- **Bump version on `qa` instead of a release branch** (option (a) in the [original backlog entry](../tasks/phase-backlog.md)). Simpler — the bump rides the existing `qa → main` merge bubble. Rejected because if a release is aborted, `qa` carries a partial version-bump commit that has to be reverted. The release branch isolates the bump until the release commits.
...
- [phase-backlog.md:121](../tasks/phase-backlog.md) — the open item this ADR closes.
