...
> **Superseded by the 2026-05-29 Amendment (see below).** The "End of Phase 9 → `1.0.0`" row reflects the *original* phasing, where Phase 9 was the launch phase. The 2026-05-22 Phase 9 / 10 / 11 / 12 restructure split implementation (Phase 9) from launch (now Phase 11), so the `v1.0.0` anchor moved to the **Phase 10 Wave F → Phase 11 cut-over** boundary. Phase 9 completion shipped an intermediate **`v0.5.0`** (2026-05-29) instead. The minor-per-phase rule (`0.MINOR.0`) extends through Phases 9–10; `v1.0.0` remains the multi-tenant launch.
