...
- [ ] **Mobile pattern** — add-relative + edit-person forms render in a bottom-sheet (`<Sheet side="bottom">` from shadcn) on mobile, side-drawer on desktop. Reference: [`../ux/inspiration/kintree/`](../ux/inspiration/kintree/) → screen "Add Relative". *(Sub-task 2 landed the mobile-bottom-sheet half via `PersonForm.tsx` — commit `1b01a1c`. Desktop intentionally uses centered `Dialog` (not a side-drawer) to match Phase 2's `CreateTreeModal` chrome for visual consistency; revisit if the side-drawer pattern reads better once edit-person lands in sub-task 3. Edit-person form still pending — leaving unchecked.)*
