...
- [ ] **Mobile pattern** — add-relative + edit-person forms render in a bottom-sheet (`<Sheet side="bottom">` from shadcn) on mobile, side-drawer on desktop. Reference: [`../ux/inspiration/kintree/`](../ux/inspiration/kintree/) → screen "Add Relative". *(Sub-task 2 landed the mobile-bottom-sheet half via `PersonForm.tsx` — commit `1b01a1c`. Sub-task 3 extended the same `PersonForm` to edit-mode so the edit-person flow ALSO goes through Sheet on mobile / Dialog on desktop — pattern is consistently applied. Still left `[ ]` because the spec wording asks for *side-drawer on desktop* and we deliberately use a centered `Dialog` to match Phase 2's `CreateTreeModal` chrome for visual consistency. Deviation stands; the centered-Dialog choice is the only blocker to ticking this.)*
