...
- [ ] **Release flow vs. `main` branch protection** — CLAUDE.md's release steps (`docs/setup` / CLAUDE.md → "Releases") run `pnpm version patch` on the freshly-pulled `main`, then `git push origin main --follow-tags`. The `main` ruleset (id `16283379`) rejects that push because it's a non-PR commit, so the v0.0.4 release on 2026-05-12 had to route the version bump through a tiny `release/v0.0.4 → main` PR ([#4](https://github.com/SanchitB23/meetthefam/pull/4)) — produced an extra merge bubble + a branch-and-PR round-trip per release. Three options to pick from before the next phase release: (a) move `pnpm version patch` to **qa** before opening the phase PR so the version bump rides the merge bubble already in the flow; (b) keep the release-branch detour and codify it in CLAUDE.md as the official step (lose nothing, document it); (c) flip the ruleset to allow the maintainer to push version-bump-only commits (e.g. allow `package.json`-only diffs from the active account — fragile to enforce, probably not worth it). My read: (a) is cleanest. Decide + update [`CLAUDE.md`](../../CLAUDE.md) → "Releases" before the v0.0.5 / Phase 4 release.
