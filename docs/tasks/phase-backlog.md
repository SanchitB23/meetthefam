...
- [ ] **(Discussion)** **Visual gender hint on avatars** — user request from Phase 3 local-browser testing: would be nice to glance at an avatar and infer gender at a glance. Tones are explicitly NOT for demographic encoding (per [`../ux/avatars-and-tones.md`](../ux/avatars-and-tones.md) — tones come from `hash(full_name) % 5` deterministically), so this would have to be a *separate* visual treatment layered on top. Options to weigh: (a) a tiny gender-icon overlay in the avatar's bottom-right (♀ / ♂ / ⚲ glyphs or Lucide icons); (b) a thin colored ring around the avatar drawn from a different palette than the 5 tones; (c) a small text-pill near the name; (d) ship without a gender hint and let `family-chart`'s own gender-aware node styling from Phase 4 carry the signal. **Not a commitment to ship** — user framed it as "I'd want this in the future maybe, take this up as a discussion point in phase 8." Decide as part of the Phase 8 polish pass; pin the call in a new ADR if the chosen option is non-trivial.
