...
- [ ] **Sub-task 2** — PersonNode go/no-go spike + custom HTML card. Resolves the [phase-backlog](phase-backlog.md) spike: does family-chart's `cardInnerHtmlCreator` cleanly support a fixed-size (~158×110) HTML node carrying our `<Avatar>` + serif name + b./d. years? If yes → ship the PersonNode pattern from [ADR 0008](../adrs/0008-design-system.md) with `setCardXSpacing(220)` / `setCardYSpacing(130)` tuning. If no → fall back to a default node style with CSS token overrides and write `docs/adrs/0011-family-chart-default-node-fallback.md`. Deceased † badge, gender-shape variation, branch/sparkle decorations all stay out of scope (Phase 8 polish per ADR 0008).
- [ ] **Sub-task 3** — Tap → person detail bottom sheet. New `_components/PersonDetailSheet.tsx` — `Sheet side="bottom"` on mobile, `side="right"` on desktop via `useIsDesktop`; Avatar (lg) + full_name + italic nickname + bio + birth_year + location + occupation + relations summary (hoist `buildRelations` from the deleted `PersonCard` into `_lib/relations.ts`); "Edit" button → shared `<PersonForm mode="edit">`. `FamilyTree.tsx` owns `[detailPersonId, setDetailPersonId]`, wires `onCardClick: (e, d) => setDetailPersonId(d.data.id)` to override the library's default click-to-recenter (re-centering moves to the sub-task 4 action menu). `page.tsx` passes a `peopleById` Map so the sheet can resolve relations without re-querying.
